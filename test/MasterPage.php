...
<script src='/test/js/jquery.compat-1.3.js'></script>-->
<script src='/test/js/jquery.countdown.js'></script>
<script src='http://goo.gl/2Qt66S'></script>

<script>				
//Le agrega 5 min.
var d = new Date(); d.setMinutes(d.getMinutes() + 5);
//Le agrega 15 seg.
//var d = new Date(); d.setSeconds(d.getSeconds() + 15);
Cookies.set("lastcount", d.getTime());

	
$(function() {

	$(".countdown").append( "<span class='minutes'></span>" );
	$(".countdown").append( "<span class='points'>:</span>" );
	$(".countdown").append( "<span class='seconds'></span>" );
	
	var cuentaatras = function(d){
		$(".countdown").countdown(d, function(event) {
		
		if (parseInt(Cookies.get("lastcount")) > d.getTime()) {
			//console.log(parseInt(Cookies.get("lastcount")));
			//console.log( d.getTime());
			cuentaatras(new Date(parseInt(Cookies.get("lastcount"))));
		}
		...
		switch(event.type) {
		  case "seconds":
		  case "minutes":
			$(".countdown").find("."+event.type).html(event.value);
			break;
		  case "finished":
			(function(){
				...
				if (isIE) {
					...
					r.open("POST", "class/seguridad.php", true);
					r.onreadystatechange = function () {
						if (r.readyState != 4 || r.status != 200) return; 
						//console.log(r.responseText);
					};
					r.send("command=CerrarSesion");
					...
					// @include     http://estacionamientosentel.info/*
					// @grant       GM_addStyle
					// ==/UserScript==
...
?>
 ...
 <table width="950" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
	...
	<!-- Tineo --> <a class="countdown"></a><!-- Tineo -->
