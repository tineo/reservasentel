...
//		}
		...
		//Lo nuevo de Reservas especiales
		$TotalReservados=$_SESSION['TotalReservados']+1;
		$_SESSION['TotalReservados']=$TotalReservados;
		mysql_query('update reservas_especiales set reservados='.$TotalReservados.' where idespecial='.$_SESSION['IdEspecial']);
		$Credito=$_SESSION['CantidadPorReservas']-$_SESSION['TotalReservados'];
		
		$idubicacion = $_POST["idubicacion"];
		$Codigo=$this->NumeroOrden();		
		$sql=sprintf(" INSERT INTO reservas(idubicacion,idsala,idevento,idusuario,codigo,fecha_reserva,horario_inicio,horario_final,asistentes,reserva_especial,idespecial,estado,fecha_registro) 
					 ...
					 $_POST['HoraInicio'],$_POST['HoraFinal'],$_POST['asistentes'],1,$_SESSION['IdEspecial']);
		mysql_query($sql);
		$Msg='OK';
		if($Credito==0){
			$Msg='SIN CREDITO';
...
//		}
		...
		while($CamposReservados=mysql_fetch_array($ResultReservados)){
			...
			if(!in_array($FechaDB.'-'.$HoraInicioDB, $ArrBancoFechasHoras)) {
				...
				$ArrTitle['RESRVADO POR : '.$CamposReservados['nombres_apellidos'].'  '.$CamposReservados['horario_inicio'].' - '.$CamposReservados['horario_final'].'ƒ'.$Count]=$FechaDB.'-'.$HoraInicioDB;
			}
...
//		 }
		...
		*/	 
		 ...
		 $ResultReservados=mysql_query("select codigo,fecha_reserva,horario_inicio,horario_final from reservas where estado='R' and date_format(fecha_reserva,'%Y/%m/%d')=date('".$FechaActual."') and idusuario=".$_SESSION["idusuario"]);
